export default {
  // Common
  common: {
    enabled: 'Abilitato',
    disabled: 'Disabilitato',
    save: 'Salva',
    cancel: 'Annulla',
    close: 'Chiudi',
    loading: 'Caricamento...',
    changing: 'Modifica in corso...',
    error: 'Errore',
    success: 'Successo',
    yes: 'Sì',
    no: 'No',
    ok: 'OK',
    unknownError: 'Errore sconosciuto',
    copyFailed: 'Copia non riuscita',
    clear: 'Cancella',
    network: {
      serverErrorTitle: 'Errore del server',
      serverError: 'Errore del server: {status}',
      timeoutTitle: 'Timeout della richiesta',
      timeout: 'La richiesta ha richiesto troppo tempo. Riprovare.',
      connectionErrorTitle: 'Errore di connessione',
      connectionError: 'Impossibile connettersi al dispositivo. Controllare la rete.'
    }
  },

  // App-level chrome (footer copyright, sponsor button)
  app: {
    footerCopyright: 'HB-RF-ETH-ng {version} © 2025-2026 Xerolux',
    sponsor: 'Sostieni',
    followOnX: 'Segui su X'
  },

  updateSuccess: {
    title: 'Aggiornamento riuscito',
    message: 'Il firmware è stato aggiornato con successo alla versione {version}.'
  },

  // Header Navigation
  nav: {
    home: 'Home',
    settings: 'Impostazioni',
    firmware: 'Firmware',
    updates: 'Updates',
    documentation: 'Documentazione',
    monitoring: 'Monitoraggio',
    systemlog: 'Registro di sistema',
    about: 'Informazioni',
    login: 'Accedi',
    logout: 'Esci',
    toggleTheme: 'Cambia tema',
    language: 'Lingua',
    mainMenu: 'Menu principale',
    groupOverview: 'Panoramica',
    groupSystem: 'Sistema',
    groupInfo: 'Info'
  },

  // Login Page
  login: {
    title: 'Effettua l\'accesso',
    subtitle: 'Inserisci nome utente e password per continuare',
    username: 'Nome utente',
    usernamePlaceholder: 'Utente',
    password: 'Password',
    passwordPlaceholder: 'Password',
    login: 'Accedi',
    loginFailed: 'Accesso fallito',
    invalidCredentials: 'Credenziali non valide',
    loginError: 'L\'accesso non è riuscito.',
    usernameRequired: 'Il nome utente è obbligatorio',
    passwordRequired: 'La password è richiesta',
    loggingIn: 'Accesso in corso...',
    forgotPassword: 'Password dimenticata?',
    passwordResetTitle: 'Reimposta password',
    passwordResetPressButton: 'Premi ora brevemente il pulsante HM sulla scheda (GPIO34 / HM_BTN) per confermare l\'accesso fisico.',
    passwordResetExpires: 'Finestra temporale: {seconds} secondi',
    passwordResetConfirmed: 'Pulsante scheda rilevato',
    passwordResetEnterNew: 'Imposta ora una nuova password amministratore.',
    newPasswordPlaceholder: 'Nuova password',
    confirmPasswordPlaceholder: 'Ripeti la nuova password',
    passwordResetSave: 'Salva password',
    passwordResetPolicy: 'Da 8 a massimo 32 caratteri con maiuscole, minuscole e un numero.',
    passwordResetMismatch: 'Le password non corrispondono.',
    passwordResetExpired: 'La finestra temporale è scaduta. Riprova dall\'inizio.',
    passwordResetError: 'Il ripristino della password non è riuscito.',
    footerCopyright: '© Xerolux 2026',
    githubLink: 'GitHub'
  },

  // Settings Page
  settings: {
    ledPatternOff: 'Spento',
    ledPatternOn: 'Acceso',
    ledPatternBlink: 'Lampeggio',
    ledPatternBlinkInv: 'Lampeggio invertito',
    ledPatternFastBlink: 'Lampeggio veloce',
    ledPatternSlowBlink: 'Lampeggio lento',
    ledPatternBlink2x: 'Lampeggio 2x',
    ledPatternBlink3x: 'Lampeggio 3x',
    ledPatternBreathing: 'Respirazione',
    ledPatternHeartbeat: 'Battito cardiaco',
    ledPatternStrobe: 'Strobo',
    ledProgramIdle: 'Inattivo',
    ledProgramCcuDisconnected: 'CCU disconnessa',
    ledProgramCcuConnected: 'CCU connessa',
    ledProgramUpdateAvailable: 'Aggiornamento disponibile',
    ledProgramError: 'Errore',
    ledProgramBooting: 'Avvio',
    ledProgramUpdateInProgress: 'Aggiornamento in corso',
    unsavedLeave: 'Hai modifiche non salvate. Uscire comunque?',
    unsavedChanges: 'Modifiche non salvate',
    allSaved: 'Salvato',
    selected: 'Selezionato: {name}',
    discardConfirm: 'Annullare le modifiche non salvate e ripristinare gli ultimi valori caricati?',
    discard: 'Scarta',
    restartError: 'Errore durante il riavvio del dispositivo',
    title: 'Impostazioni',
    ccuSettings: 'Connessione CCU',
    ccuIpAddress: 'Indirizzo IP CCU',
    ccuIpHint: 'Inserisci l\'indirizzo IP della tua CCU per evitare il blocco della connessione. Il sistema si riavvierà dopo il salvataggio.',
    tabGeneral: 'Generale',
    tabNetwork: 'Rete',
    tabTime: 'Ora',
    tabBackup: 'Backup',
    tabDesign: 'Design',
    tabLicense: 'Licenza',
    tabExperimental: 'Sperimentale',
    advancedTitle: 'Avanzate',
    showExperimental: 'Mostra funzioni sperimentali',
    showExperimentalHint: 'Mostra voci di menu, schede e impostazioni sperimentali. I valori sperimentali già salvati vengono mantenuti quando vengono nascosti di nuovo.',
    showExperimentalWarning: 'Le funzioni sperimentali sono incomplete e possono cambiare o non funzionare come previsto.',
    showExperimentalOn: 'Funzioni sperimentali visibili',
    showExperimentalOff: 'Funzioni sperimentali nascoste',
    changePassword: 'Cambia Password',
    repeatPassword: 'Ripeti Password',
    hostname: 'Nome Host',

    // Security Settings
    security: 'Sicurezza',
    adminUsername: 'Nome utente amministratore',
    adminUsernameHint: 'Il valore predefinito è admin. Può essere modificato per i gestori di password o più dispositivi.',
    changePasswordHint: 'Cambia la password di amministratore per un accesso sicuro',
    otaPassword: 'Password OTA',
    otaPasswordHint: 'Password separata richiesta per gli aggiornamenti del firmware',
    changePasswordBtn: 'Cambia Password',
    changeOtaPassword: 'Cambia',
    setOtaPassword: 'Imposta Password',
    clearOtaPassword: 'Rimuovi',
    clearOtaPasswordConfirm: 'Sei sicuro di voler rimuovere la password OTA? Gli aggiornamenti firmware non saranno possibili finché non verrà impostata una nuova password.',
    clearOtaPasswordSuccess: 'La password OTA è stata rimossa.',
    clearOtaPasswordError: 'Impossibile rimuovere la password OTA',

    // Network Settings
    networkSettings: 'Impostazioni di Rete',
    dhcp: 'DHCP',
    ipAddress: 'Indirizzo IP',
    netmask: 'Maschera di rete',
    gateway: 'Gateway',
    dns1: 'DNS Primario',
    dns2: 'DNS Secondario',

    // IPv6 Settings
    ipv6Settings: 'Impostazioni IPv6',
    enableIPv6: 'Abilita IPv6',
    ipv6Mode: 'Modalità IPv6',
    ipv6Auto: 'Automatico (SLAAC)',
    ipv6Static: 'Statico',
    ipv6Address: 'Indirizzo IPv6',
    ipv6PrefixLength: 'Lunghezza Prefisso',
    ipv6Gateway: 'Gateway IPv6',
    ipv6Dns1: 'DNS IPv6 Primario',
    ipv6Dns2: 'DNS IPv6 Secondario',

    // Time Settings
    timeSettings: 'Impostazioni Ora',
    timesource: 'Fonte Oraria',
    ntp: 'NTP',
    dcf: 'DCF',
    gps: 'GPS',
    ntpServer: 'Server NTP',
    dcfOffset: 'Offset DCF',
    gpsBaudrate: 'Baudrate GPS',

    // System Settings
    systemSettings: 'Impostazioni di Sistema',
    ledBrightness: 'Luminosità LED',
    ledPrograms: 'Programmi LED',
    ledProgramsHelp: 'Personalizza il comportamento dei LED per i diversi stati del sistema.',
    allowPrerelease: 'Consenti aggiornamenti anticipati (Beta/Alpha)',
    language: 'Lingua',

    // Messages
    saveSuccess: 'Le impostazioni sono state salvate con successo.',
    saveError: 'Si è verificato un errore durante il salvataggio delle impostazioni.',
    restartTitle: 'Riavvio Richiesto',
    restartMessage: 'Le impostazioni sono state salvate. Vuoi riavviare il dispositivo ora per applicare le modifiche?',
    restartNow: 'Riavvia Ora',
    restartLater: 'Riavvia Più Tardi',

    // Backup & Restore
    backupRestore: 'Backup e Ripristino',
    backupInfo: 'Scarica un backup delle tue impostazioni per ripristinarle in seguito.',
    restoreInfo: 'Carica un file di backup per ripristinare le impostazioni. Il sistema si riavvierà successivamente.',
    downloadBackup: 'Scarica Backup',
    restore: 'Ripristina',
    restoreBtn: 'Ripristina',
    download: 'Scarica',
    noFileChosen: 'Nessun file selezionato',
    browse: 'Sfoglia',
    restoreConfirm: 'Sei sicuro? Le impostazioni attuali verranno sovrascritte e il sistema si riavvierà.',
    experimentalTitle: 'Sperimentale',
    experimentalEmptyTitle: 'Niente qui per ora',
    experimentalEmptyText: 'Al momento non ci sono funzioni sperimentali disponibili. Il nuovo design e la sincronia di riavvio sono uno standard per tutti i dispositivi.',
    experimentalWarningTitle: 'Lavori in corso',
    experimentalWarningText: 'Queste funzioni sono pensate per i test. Non viene fornita alcuna garanzia sul funzionamento o sull’aspetto.',
    experimentalDesign: 'Layout sperimentale',
    experimentalDesignHint: 'Attiva il nuovo layout sperimentale. La scelta viene salvata sul dispositivo e resta attiva dopo il riavvio.',
    flashPause: 'Sincronia riavvio',
    flashPauseHint: 'Dopo un riavvio causato da un aggiornamento firmware o da una modifica delle impostazioni, scollega il collegamento Ethernet per 35 s prima del riavvio, in modo che il watchdog della CCU rilevi la disconnessione e riavvii in modo pulito.',
    restoreSuccess: 'Impostazioni ripristinate con successo. Riavvio del sistema...',
    restoreError: 'Errore durante il ripristino delle impostazioni',
    backupError: 'Errore durante il download del backup',
    validation: {
      invalidIpv4OrIpv6: 'Indirizzo IPv4 o IPv6 non valido',
      invalidIpv6: 'Indirizzo IPv6 non valido',
      minPrefix: 'Min 1',
      maxPrefix: 'Max 128',
      fixErrors: 'Correggi i campi evidenziati.'
    }
  },

  // System Info
  sysinfo: {
    updatesRecovery: 'Aggiornamenti e ripristino',
    liveStreamExport: 'Streaming live ed esportazione',
    networkTimeSetup: 'Configurazione rete e ora',
    copied: '{label} copiato',
    title: 'Informazioni di Sistema',
    goodMorning: 'Buongiorno',
    goodAfternoon: 'Buon pomeriggio',
    goodEvening: 'Buonasera',
    serial: 'Numero di Serie',
    hostname: 'Nome Host',
    boardRevision: 'Revisione Scheda',
    uptime: 'Tempo di attività',
     // Value is the cause of the last reboot, not a timestamp (Korrekturauftrag §1.5).
     resetReason: 'Motivo riavvio',
    cpuUsage: 'Utilizzo CPU',
    memoryUsage: 'Utilizzo memoria',
    ethernetStatus: 'Connessione Ethernet',
    rawUartRemoteAddress: 'Connesso alla CCU',
    localIP: 'Indirizzo IPv4',
    netmask: 'Maschera di sottorete',
    gateway: 'Gateway',
    dns: 'Server DNS',
    ipv6Addresses: 'Indirizzi IPv6',
    radioModuleType: 'Tipo Modulo Radio',
    radioModuleSerial: 'Numero di Serie',
    radioModuleFirmware: 'Versione Firmware',
    radioModuleBidCosRadioMAC: 'Indirizzo Radio (BidCoS)',
    radioModuleHmIPRadioMAC: 'Indirizzo Radio (HmIP)',
    radioModuleSGTIN: 'SGTIN',
    version: 'Versione',
    latestVersion: 'Ultima Versione',
    memory: 'Utilizzo memoria',
    cpu: 'Utilizzo CPU',
    temperature: 'Temperatura',
    voltage: 'Tensione di Alimentazione',
    ethernet: 'Ethernet',
    connected: 'Connesso',
    disconnected: 'Disconnesso',
    speed: 'Velocità',
    duplex: 'Duplex',
    radioModule: 'Modulo Radio',
    moduleType: 'Tipo Modulo',
    firmwareVersion: 'Versione Firmware',
    bidcosMAC: 'MAC Radio BidCoS',
    hmipMAC: 'MAC Radio HmIP',
    system: 'Sistema',
    network: 'Rete',
    down: 'Giù',
    mbits: 'Mbit/s',
    online: 'Online',
    offline: 'Offline',
    dashboardTitle: 'Stato del Sistema',
    monitoringSummary: 'MQTT, CheckMK, Prometheus, Syslog e notifiche',
    updateAvailableShort: 'Aggiorna {version}',
    lan: 'LAN',
    boardSenseHint: 'Lettura grezza dell\'ADC dal pin di rilevamento della revisione scheda. Il confronto con le quattro tensioni nominali del partitore identifica la revisione: Rev 1.10/1.11 PUB ≈ 550 mV, Rev 1.8 SK ≈ 1650 mV, Rev 1.8 PUB ≈ 2750 mV, Rev 1.10/1.11 SK ≈ 3050 mV.'
  },

  // System Overview page (dashboard / Diagnostics)
  systemOverview: {
    eyebrow: 'Diagnostica',
    title: 'Panoramica del sistema',
    subtitle: 'RAM, flash, partizioni, log e interfaccia web attiva a colpo d’occhio.',
    freeRam: 'RAM libera',
    usedRam: 'RAM usata',
    totalRam: 'RAM totale',
    minimumHeap: 'RAM minima',
    largestBlock: 'Blocco più grande',
    flash: 'Flash',
    currentAvailable: 'Attualmente disponibile',
    allocatableRam: 'RAM interna disponibile per l\'heap',
    sinceBoot: 'Valore più basso dall\'avvio',
    contiguous: 'Più grande allocazione contigua',
    physicalFlash: 'Memoria flash rilevata',
    runtime: 'Ambiente di runtime',
    runtimeHint: 'ESP-IDF, hardware e ultimo riavvio',
    idf: 'ESP-IDF',
    target: 'Piattaforma di destinazione',
    cores: 'Core CPU',
    chipRevision: 'Revisione chip',
    resetReason: 'Ultimo motivo di reset',
    psram: 'PSRAM',
    psramTotal: 'PSRAM totale',
    psramFree: 'PSRAM libera',
    partitions: 'Partizioni OTA',
    partitionsHint: 'Partizione firmware attiva e successiva',
    runningPartition: 'Attiva',
    runningSize: 'Dimensione attiva',
    nextPartition: 'Prossimo aggiornamento',
    nextSize: 'Dimensione aggiornamento',
    webuiHint: 'Origine e archivio del nuovo design',
    source: 'Origine',
    version: 'Versione',
    partitionSize: 'Dimensione partizione',
    used: 'Usato',
    logs: 'Buffer di log',
    logsHint: 'Memoria, crash tail e stream live attivi',
    capture: 'Acquisizione',
    active: 'Attivo',
    inactive: 'Inattivo',
    bufferSize: 'Dimensione buffer',
    availableLogs: 'Log memorizzati',
    totalStream: 'Totale scritto',
    subscribers: 'Sottoscrittori',
    crashTail: 'Crash tail',
    available: 'Disponibile',
    notAvailable: 'Non disponibile',
    onDemandHint: 'Tutti i valori vengono letti solo all\'apertura o all\'aggiornamento manuale. La pagina di emergenza indipendente è disponibile su',
    refresh: 'Aggiorna'
  },

  // Update
  update: {
    available: 'Aggiornamento Disponibile',
    updateNow: 'Aggiorna Ora'
  },

  // Updates page (merged Firmware + WebUI entry, Korrekturauftrag §6)
  updates: {
    eyebrow: 'Aggiornamenti',
    title: 'Aggiornamenti',
    subtitle: 'Il firmware e l\'interfaccia web hanno versioni separate e vengono aggiornati in modo indipendente.',
    firmware: 'Firmware',
    webui: 'WebUI',
    checkNow: 'Cerca aggiornamenti ora',
    checkingNow: 'Ricerca in corso …',
    checkResultUpdatedTitle: 'Aggiornamento disponibile',
    checkResultUpdated: 'La versione {version} è disponibile.',
    checkResultNoUpdateTitle: 'Tutto aggiornato',
    checkResultNoUpdate: 'Non è disponibile una versione più recente.',
    checkResultErrorTitle: 'Verifica non riuscita',
    checkResultError: 'La verifica degli aggiornamenti non è stata completata.',
    checkResultCooldownTitle: 'Un momento',
    checkResultCooldown: 'Una verifica è già in corso oppure il blocco di 60 secondi è ancora attivo.',
    checkResultSkippedTitle: 'Verifica saltata',
    checkResultSkipped: 'Il dispositivo ha saltato la verifica per memoria libera insufficiente. Questo accade di solito mentre è attiva una sessione CCU — riprova più tardi.'
  },

  // Pagina di aggiornamento WebUI (scheda WebUI separata)
  webuiUpdate: {
    pageTitle: 'Aggiorna l\'interfaccia web',
    pageSubtitle: 'Il firmware e la WebUI hanno versioni separate. L\'installazione avviene esclusivamente tramite un\'immagine WWW di 320 KB selezionata manualmente.',
    webuiChip: 'WebUI',
    firmwareChip: 'Firmware',
    separateActive: 'WebUI separata attiva',
    embeddedFallback: 'Fallback integrato attivo',
    installedLabel: 'Versione WebUI installata',
    separateSource: 'Dalla partizione WWW separata',
    embeddedSource: 'Integrata nel firmware',
    storageLabel: 'Memoria WWW',
    checkLabel: 'Verifica aggiornamento',
    checkHint: 'Interrogazione online al massimo una volta ogni 24 ore',
    availableKicker: 'WebUI disponibile',
    downloadHeading: 'Scarica file WebUI',
    downloadHelp: 'L\'ESP scarica solo il piccolo manifesto di aggiornamento condiviso. Il file BIN viene scaricato direttamente dal tuo browser.',
    installedGridLabel: 'Installata',
    availableGridLabel: 'Disponibile',
    imageSizeLabel: 'Dimensione immagine',
    minFirmwareLabel: 'Firmware minimo',
    upToDate: 'La WebUI installata è aggiornata.',
    downloadButton: 'Scarica BIN WebUI',
    releaseLink: 'Release su GitHub',
    reloadStatusButton: 'Ricarica stato salvato',
    manualKicker: 'Installazione manuale',
    uploadHeading: 'Carica immagine WWW',
    uploadHelp: 'Usa qui esclusivamente webui_*.bin o spiffs.bin con esattamente {size}.',
    chooseFile: 'Scegli BIN WebUI',
    uploadInProgress: 'Trasferimento WebUI …',
    uploadExplainer: 'Il dispositivo verifica la dimensione della partizione, il manifesto interno ed eventualmente SHA-256. Se l\'immagine non è valida o incompleta, resta attivo il New Design integrato come fallback.',
    installingButton: 'Installazione WebUI …',
    installButton: 'Installa WebUI',
    clearSelectionButton: 'Cancella selezione',
    unknownVersion: 'sconosciuta',
    neverChecked: 'Non ancora verificato',
    statusLoadError: 'Impossibile caricare lo stato del dispositivo.',
    manifestNoWebui: 'Il manifesto di aggiornamento salvato non contiene ancora un file WebUI separato.',
    manifestWrongDesign: 'L\'immagine WebUI fornita non appartiene al New Design.',
    manifestWrongSize: 'La WebUI offerta ha una dimensione errata. Attesi {expected}.',
    manifestWrongApi: 'Questa WebUI richiede un\'API firmware diversa.',
    manifestFirmwareTooOld: 'Il firmware è troppo vecchio. Richiesto almeno {min}.',
    manifestLoadError: 'Impossibile caricare lo stato di aggiornamento salvato.',
    updateAvailableTitle: 'Aggiornamento disponibile',
    updateAvailableMessage: 'La versione {version} della WebUI è disponibile.',
    skippedTitle: 'Verifica saltata',
    skippedMessage: 'Il dispositivo ha saltato la verifica per memoria libera insufficiente. Avviene di solito mentre è attiva una sessione CCU — riprova più tardi.',
    upToDateTitle: 'Tutto aggiornato',
    upToDateMessage: 'Nessuna WebUI più recente disponibile.',
    cooldownTitle: 'Un momento',
    cooldownMessage: 'Una verifica è già in corso o il blocco di 60 secondi è ancora attivo.',
    checkFailedTitle: 'Verifica non riuscita',
    checkFailedMessage: 'Impossibile completare la verifica dell\'aggiornamento.',
    fileInvalidExtension: 'File non valido: è richiesta un\'immagine WebUI con estensione .bin.',
    fileIsFirmware: 'File errato: è un file firmware. Installarlo in Sistema → Firmware.',
    fileStatusMissing: 'Stato del dispositivo non disponibile. Esegui prima “Ricarica stato salvato” e riprova.',
    fileWrongSize: 'Dimensione immagine errata: attesi esattamente {expected} ({bytes} byte).',
    installSuccessTitle: 'WebUI installata',
    installSuccessMessage: 'Versione WebUI attiva: {version}',
    uploadFailedTitle: 'Caricamento WebUI non riuscito',
    uploadFailedMessage: 'Caricamento WebUI non riuscito.'
  },

  // Firmware Update
  firmware: {
    jumpToOta: 'Vai agli aggiornamenti OTA',
    lastCheckAt: 'Ultimo controllo: {time}',
    noRecentCheck: 'Nessun controllo recente',
    title: 'Firmware',
    subtitle: 'Aggiorna il firmware del dispositivo',
    restarting: 'Riavvio in corso...',
    restartingText: 'Il dispositivo si sta riavviando. La pagina si ricaricherà automaticamente.',
    fileUpload: 'Caricamento File',
    fileUploadHint: 'Carica un file firmware .bin',
    networkUpdate: 'Aggiornamento di Rete',
    networkUpdateHint: 'Scarica da URL',
    urlPlaceholder: 'https://esempio.com/firmware.bin',
    downloading: 'Download in corso...',
    downloadInstall: 'Scarica e Installa',
    factoryReset: 'Ripristino di fabbrica',
    factoryResetHint: 'Ripristina alle impostazioni di fabbrica',
    factoryResetTitle: 'Ripristino di fabbrica',
    factoryResetMessage: 'Vuoi davvero ripristinare tutte le impostazioni ai valori di fabbrica?',
    factoryResetWarning: 'Tutte le impostazioni salvate andranno perse. Il dispositivo si riavvierà automaticamente al termine.',
    factoryResetConfirm: 'Ripristina impostazioni di fabbrica',
    factoryResetError: 'Ripristino di fabbrica non riuscito.',
    otaProgress: 'Download aggiornamento firmware...',
    otaSuccess: 'Download completato. Installazione in corso...',
    currentVersion: 'Versione Attuale',
    installedVersion: 'Versione Installata',
    versionInfo: 'Fork modernizzato v2.2.5-Beta.8 di Xerolux (2025) - Basato sul lavoro originale di Alexander Reinert.',
    updateAvailable: 'Aggiornamento disponibile!',
    updateAvailableMessage: 'È disponibile un aggiornamento alla versione {latestVersion}.',
    upToDate: 'Il firmware è aggiornato',
    checkFailed: 'Verifica aggiornamento non riuscita',
    lastCheck: 'Ultimo controllo',
    newVersionAvailable: 'Nuova versione {version} disponibile!',
    viewUpdate: 'Vedi',
    onlineUpdate: 'Aggiorna Online',
    onlineUpdateConfirm: 'Vuoi davvero scaricare e installare l\'aggiornamento? Il sistema si riavvierà automaticamente.',
    onlineUpdateStarted: 'Aggiornamento avviato. Il dispositivo si riavvierà automaticamente una volta terminato.',
    showReleaseNotes: 'Mostra Note di Rilascio',
    releaseNotesTitle: 'Note di Rilascio per v{version}',
    releaseNotesError: 'Impossibile caricare le note di rilascio da GitHub.',
    updateFile: 'File Firmware',
    noFileChosen: 'Nessun file scelto',
    browse: 'Sfoglia',
    selectFile: 'Seleziona o trascina file .bin',
    upload: 'Installa Firmware',
    restart: 'Riavvia',
    restartHint: 'Riavvia dispositivo',
    uploading: 'Caricamento...',
    uploadSuccess: 'Aggiornamento firmware caricato con successo. Il sistema si riavvierà automaticamente tra 3 secondi...',
    uploadError: 'Si è verificato un errore.',
    updateSuccess: 'Firmware aggiornato con successo',
    updateError: 'Errore durante l\'aggiornamento del firmware',
    warning: 'Attenzione: Non scollegare l\'alimentazione durante l\'aggiornamento!',
    restartConfirm: 'Vuoi davvero riavviare il sistema?',
    beta: 'Beta',
    betaChannel: 'Aggiornamenti beta',
    betaChannelHint: 'Includi le versioni preliminari. Le versioni beta possono essere instabili.',
    betaChannelOn: 'Aggiornamenti beta attivati – verifica completata',
    betaChannelOff: 'Aggiornamenti beta disattivati – verifica completata',
    releaseNotesPreview: 'Note di rilascio',
    viewOnGithub: 'Vedi su GitHub',
    noDownloadUrl: 'Nessun binario firmware collegato a questa versione. Riprova più tardi o caricalo manualmente.',
    archiveTitle: 'Archivio firmware',
    archiveHint: 'Installa una versione precedente, passa da beta a stabile o testa una versione specifica.',
    archiveFilter: 'Filtro archivio firmware',
    archiveSelect: 'Seleziona versione firmware',
    archiveStable: 'Stabile',
    archiveBeta: 'Beta',
    archiveAll: 'Tutte',
    archiveLoading: 'Caricamento versioni...',
    archiveWarning: 'Crea un backup prima di cambiare firmware. Le versioni più vecchie potrebbero non comprendere le impostazioni più recenti.',
    archiveLoadError: 'Impossibile caricare l’archivio firmware',
    archiveEmpty: 'Nessuna versione firmware corrispondente trovata.',
    archiveCurrent: 'Installata',
    archiveInstalled: 'Installata',
    archiveInstall: 'Installa',
    archiveInstallConfirm: 'Installare la versione firmware {version}? Il dispositivo la scaricherà online e si riavvierà automaticamente. Crea prima un backup.',
    archiveReleaseNotes: 'Changelog / note di rilascio',
    otaFailed: 'Aggiornamento OTA non riuscito',
    lostConnection: 'Connessione al dispositivo persa durante l\'aggiornamento',
    restartFlashPauseHint: 'Sincronia riavvio attiva: il collegamento Ethernet verrà mantenuto scollegato per circa 35 s prima del riavvio effettivo del dispositivo. Il ricaricamento della pagina richiederà quindi un po\' più di tempo del solito.'
  },

  // Monitoring
  monitoring: {
    selfTestHint: 'Esegui un rapido autotest per verificare la configurazione attuale.',
    selfTestTitle: 'Autotest',
    testButton: 'Test',
    checkFinished: 'Controllo completato',
    diagnosticFailed: 'Richiesta di diagnostica non riuscita',
    chipLabelCheckmk: 'CheckMK',
    chipLabelMqtt: 'MQTT',
    chipLabelPrometheus: 'Prometheus',
    chipLabelSyslog: 'Syslog',
    chipLabelNotify: 'Notifiche',
    title: 'Monitoraggio',
    description: 'Configura monitoraggio e notifiche per il gateway HB-RF-ETH.',
    resourceWarningTitle: 'Tieni d\'occhio le risorse',
    resourceWarningText: 'Ci sono {count} servizi di monitoraggio attivi. L\'ESP32 di norma riesce a gestirli, ma connessioni TLS simultanee, ricerca/archivio firmware e OTA competono per heap e CPU; in caso di problemi disattiva singoli servizi o dilaziona i test nel tempo.',
    save: 'Salva',
    saving: 'Salvataggio...',
    saveSuccess: 'Configurazione salvata con successo!',
    saveError: 'Errore durante il salvataggio della configurazione!',
    checkmk: {
      title: 'Agente CheckMK',
      enabled: 'Abilita CheckMK',
      port: 'Porta',
      portHelp: 'Predefinito: 6556',
      allowedHosts: 'Host consentiti',
      allowedHostsHelp: 'Indirizzi IP separati da virgola (es. "192.168.1.10,192.168.1.20") oppure "*" per consentire tutti'
    },
    mqtt: {
      title: 'Client MQTT',
      enabled: 'Abilita MQTT',
      server: 'Server',
      serverHelp: 'Hostname o IP del Broker MQTT',
      port: 'Porta',
      portHelp: 'Predefinito: 1883 (8883 per TLS/SSL)',
      user: 'Utente',
      userHelp: 'Opzionale: Nome Utente MQTT',
      password: 'Password',
      passwordHelp: 'Opzionale: Password MQTT',
      topicPrefix: 'Prefisso Topic',
      topicPrefixHelp: 'Segmento radice di ogni topic MQTT pubblicato/sottoscritto da questo dispositivo. Predefinito: hb-rf-eth-ng. Esempi: "prefisso/status/uptime", "prefisso/status/version", "prefisso/command/update". Visibile nel broker MQTT (es. MQTT Explorer), non nell\'interfaccia Web.',
      haDiscoveryEnabled: 'Home Assistant Discovery',
      haDiscoveryPrefix: 'Prefisso Discovery',
      haDiscoveryPrefixHelp: 'Predefinito: homeassistant',
      commands: {
        title: 'Topic dei comandi',
        enableHelp: 'Consente il controllo remoto (riavvio, ripristino di fabbrica, OTA) tramite MQTT. Senza token o ACL del broker, qualsiasi client sulla rete può controllare il dispositivo.',
        token: 'Token comando',
        tokenPlaceholder: 'opzionale – segreto condiviso',
        tokenHelp: 'Se impostato, il payload del comando deve corrispondere esattamente a questo token. Consentiti: A-Z a-z 0-9 - _ .',
        tokenPresent: '✓ Presente – inserisci un nuovo valore per sostituire',
        clear: 'Cancella',
        aclHint: 'Nota: senza ACL del broker e token, qualsiasi client MQTT può riavviare il dispositivo o avviare un aggiornamento OTA. Configura sul broker un\'ACL che conceda i diritti di pubblicazione sul topic dei comandi solo agli host autorizzati.',
      },
      serverRequired: 'Inserire un indirizzo server MQTT quando MQTT è abilitato.',
      tls: {
        title: 'TLS / SSL',
        enable: 'Abilita TLS',
        enableHelp: 'Opzionale: Proteggere la connessione MQTT tramite TLS',
        skipVerify: 'Disabilita verifica certificato',
        skipVerifyAlert: 'Attenzione: Senza verifica del certificato non è possibile rilevare attacchi man-in-the-middle. Utilizzare solo in ambienti di test o con certificati self-signed.',
        caCerts: 'Certificato CA (PEM)',
        caCertsHelp: 'Lasciare vuoto = usare il bundle CA radice Mozilla integrato nel firmware',
        certfile: 'Certificato client (mTLS, PEM)',
        certfileHelp: 'Opzionale: certificato client mTLS',
        keyfile: 'Chiave client (mTLS, PEM)',
        keyfileHelp: 'Opzionale: chiave client mTLS',
        certPresent: '✓ Presente – inserire un nuovo valore per sostituire',
        clear: 'Cancella',
        loadFromFile: 'Carica da file',
        fileTooLarge: 'File troppo grande (max. 16 KB).',
        fileReadError: 'Impossibile leggere il file.',
        pemValid: '✓ Formato PEM riconosciuto ({count} blocco | {count} blocchi).',
        pemTooLarge: 'Contenuto troppo grande (max. 8 KB per campo).',
        keySingleBlock: 'Il file di chiave deve contenere esattamente un blocco PRIVATE KEY.',
        encryptedKeyError: 'Le chiavi private crittografate non sono supportate. Esportare senza password: openssl pkcs8 -topk8 -nocrypt -in key.pem -out key-plain.pem',
        invalidPemFormat: 'Nessun contenuto PEM valido rilevato (marcatori BEGIN/END mancanti o non corrispondenti).',
        unexpectedPemLabel: 'Tipo di blocco PEM inatteso per questo campo.',
        binaryFormatError: 'Rilevato formato binario (DER) – convertire prima: openssl x509 -inform DER -in cert.der -out cert.pem',
        pkcs12Error: 'Il contenitore PKCS#12 (.p12/.pfx) non è supportato. Estrarre: openssl pkcs12 -in bundle.p12 -nodes -out cert.pem',
        invalidBase64: 'Il corpo PEM contiene caratteri non validi.',
        mtlsRequiresBoth: 'Per mTLS devono essere forniti sia il certificato che la chiave.'
      }
    },
    enable: 'Abilita',
    allowedHosts: 'Host Consentiti',
    prometheus: {
      title: 'Esportatore Prometheus',
      port: 'Porta',
      portHelp: 'Predefinito: 9100',
      allowedHosts: 'Host consentiti',
      allowedHostsHelp: 'Indirizzi IP separati da virgola oppure "*" per consentire tutti'
    },
    syslog: {
      title: 'Inoltro Syslog',
      server: 'Server',
      serverHelp: 'Hostname o IP del server Syslog',
      port: 'Porta',
      portHelp: 'Predefinito: 514',
      transport: 'Trasporto',
      minSeverity: 'Gravità min.',
      minSeverityHelp: 'Inoltra solo i messaggi a partire da questo livello di gravità',
      hostname: 'Override hostname',
      hostnameHelp: 'Vuoto = usa l\'hostname del dispositivo'
    },
    notify: {
      title: 'Notifiche degli eventi',
      channels: 'Canali attivi',
      channelWebhook: 'Webhook',
      channelTelegram: 'Telegram',
      channelEmail: 'E-mail',
      cooldown: 'Intervallo (secondi)',
      cooldownHelp: 'Per ogni tipo di evento viene inviata una sola notifica durante questa finestra',
      webhookSection: 'Webhook',
      webhookUrl: 'URL',
      webhookSecret: 'Secret',
      telegramSection: 'Telegram',
      telegramToken: 'Token del bot',
      telegramChatId: 'Chat ID',
      smtpSection: 'E-mail (SMTP)',
      smtpServer: 'Server',
      smtpPort: 'Porta',
      smtpTls: 'Crittografia',
      smtpTlsNone: 'Nessuna',
      smtpTlsStarttls: 'STARTTLS',
      smtpTlsImplicit: 'TLS (implicito)',
      smtpUser: 'Utente',
      smtpPassword: 'Password',
      smtpFrom: 'Mittente',
      smtpTo: 'Destinatario',
      secretPresent: '✓ Salvato – inserire un nuovo valore per sostituire'
    },
    diag: {
      unsupported: 'Destinazione di diagnostica sconosciuta',
      checkmk: {
        disabled: 'CheckMK è disattivato',
        listening: 'L\'agente CheckMK è in ascolto sulla porta TCP {port}',
        not_ready: 'CheckMK è attivato, ma il listener non è pronto'
      },
      mqtt: {
        disabled: 'MQTT è disattivato',
        tcp_ok: 'Connessione TCP a {host}:{port} riuscita',
        tcp_failed: 'Connessione TCP a {host}:{port} non riuscita',
        tls_note: ' (TLS attivo, verifica dei certificati non testata)'
      },
      prometheus: {
        disabled: 'Prometheus è disattivato',
        listening: 'L\'esportatore Prometheus è in ascolto sulla porta TCP {port}',
        not_ready: 'Prometheus è attivato, ma il listener non è pronto'
      },
      syslog: {
        disabled: 'L\'inoltro Syslog è disattivato',
        tcp_ok: 'Connessione TCP a {host}:{port} riuscita',
        tcp_failed: 'Connessione TCP a {host}:{port} non riuscita'
      },
      notify: {
        disabled: 'Le notifiche sono disattivate',
        queued: 'Notifica di prova accodata per i canali 0x{mask}'
      }
    }
  },

  // About Page
  about: {
    title: 'Informazioni',
    version: 'Versione 2.2.5-Beta.6',
    fork: 'Fork Modernizzato',
    forkDescription: 'Questa versione è un fork modernizzato da Xerolux (2025), basato sul firmware originale HB-RF-ETH. Aggiornato a ESP-IDF 6.0, toolchain moderne e tecnologie WebUI attuali (Vue 3, Vite, Pinia).',
    original: 'Autore Originale',
    firmwareLicense: 'Il',
    hardwareLicense: 'Il',
    under: 'è rilasciato sotto',
    description: 'Gateway LAN HomeMatic BidCoS/HmIP',
    author: 'Autore',
    license: 'Licenza',
    website: 'Sito Web',
    documentation: 'Documentazione',
    support: 'Supporto',
    githubRepository: 'Repository GitHub',
    copyrightXerolux: 'Copyright (c) 2025, Xerolux',
    copyrightOriginal: 'Copyright (c) 2022, Alexander Reinert'
  },

  // Third Party
  thirdParty: {
    title: 'Software di Terze Parti',
    containsThirdPartySoftware: 'Questo software contiene prodotti software gratuiti di terze parti utilizzati secondo varie condizioni di licenza.',
    providedAsIs: 'Il software è fornito "così com\'è" SENZA ALCUNA GARANZIA.'
  },

  // Change Password
  changePassword: {
    title: 'Cambio Password Richiesto',
    eyebrow: 'Sicurezza',
    subtitle: 'Proteggi il tuo account',
    warningTitle: 'Richiesto',
    requirementsTitle: 'Requisiti Password:',
    reqMinLength: 'Almeno 8 caratteri',
    reqLettersNumbers: 'Deve contenere maiuscole, minuscole e numeri',
    currentPassword: 'Password Attuale',
    newPassword: 'Nuova Password',
    confirmPassword: 'Conferma Password',
    changePassword: 'Cambia Password',
    changeSuccess: 'Password cambiata con successo',
    changeError: 'Errore durante il cambio password',
    passwordMismatch: 'Le password non corrispondono',
    passwordTooShort: 'La password deve essere lunga almeno 8 caratteri e contenere maiuscole, minuscole e numeri.',
    passwordRequirements: 'Deve contenere maiuscole, minuscole e numeri',
    passwordsDoNotMatch: 'Le password non corrispondono',
    warningMessage: 'Questo è il tuo primo accesso o la password è ancora impostata su "admin". Per motivi di sicurezza, devi cambiare la password.',
    success: 'Password cambiata con successo',
    newPasswordPlaceholder: 'Inserisci nuova password',
    currentPasswordPlaceholder: 'Inserisci password attuale',
    currentPasswordRequired: 'La password attuale è richiesta',
    confirmPasswordPlaceholder: 'Conferma nuova password'
  },

  // OTA Password Modal
  otaPassword: {
    title: 'Imposta Password OTA',
    warningMessage: 'Imposta una password separata per gli aggiornamenti del firmware. Questo è richiesto per gli aggiornamenti OTA.',
    otaPassword: 'Password OTA',
    otaPasswordPlaceholder: 'Inserisci password OTA',
    confirmPassword: 'Conferma Password',
    currentPasswordPlaceholder: 'Inserisci password attuale',
    currentPasswordRequired: 'La password attuale è richiesta',
    confirmPasswordPlaceholder: 'Conferma password OTA',
    passwordTooShort: 'La password deve essere lunga almeno 8 caratteri',
    passwordRequirements: 'Deve contenere maiuscole, minuscole e numeri',
    passwordsDoNotMatch: 'Le password non corrispondono',
    requirementsTitle: 'Requisiti Password:',
    reqMinLength: 'Almeno 8 caratteri',
    reqMixedCase: 'Lettere maiuscole e minuscole',
    reqNumbers: 'Almeno un numero',
    strengthWeak: 'Debole',
    strengthMedium: 'Media',
    strengthGood: 'Buona',
    strengthStrong: 'Forte'
  },

  // Sponsor
  sponsor: {
    title: 'Supporta questo Progetto',
    description: 'Se ti piace questo progetto e vuoi supportare il suo sviluppo, puoi usare una delle opzioni qui sotto.',
    thanks: 'Grazie per il tuo supporto!',
    teslaReferral: 'Referral Tesla'
  },

  // System Log
  systemlog: {
    subtitle: 'Filtra, metti in pausa ed esporta il log live del dispositivo.',
    paused: 'In pausa',
    live: 'Live',
    lines: 'righe',
    newEntries: 'nuove',
    pause: 'Pausa',
    resume: 'Riprendi',
    manualScroll: 'Scorrimento manuale',
    copy: 'Copia',
    searchPlaceholder: 'Cerca nel log',
    allLevels: 'Tutti i livelli',
    levelError: 'Errore',
    levelWarning: 'Avviso',
    levelInfo: 'Info',
    levelDebug: 'Debug',
    noMatches: 'Nessuna riga corrispondente',
    copiedVisible: 'Log visibile copiato',
    copiedLine: 'Riga copiata',
    copyVisibleFailed: 'Impossibile copiare il log',
    copyLineFailed: 'Impossibile copiare la riga',
    title: 'Registro di sistema',
    description: 'Visualizzazione in tempo reale dell\'output del registro di sistema con funzionalità di download.',
    liveLog: 'Log in Diretta',
    autoScroll: 'Scorrimento Automatico',
    enabled: 'Abilitato',
    disabled: 'Disabilitato',
    enableFailed: 'Impossibile abilitare il buffer dei log (memoria insufficiente).',
    disabledMessage: 'Il visualizzatore di log è disabilitato. Accendi l\'interruttore per vedere i log in diretta.',
    refresh: 'Aggiorna',
    clear: 'Pulisci',
    download: 'Scarica',
    empty: 'Nessuna voce di log ancora.',
    crashTitle: 'Log di crash recuperato',
    crashHint: 'Il dispositivo si è riavviato dopo un reset del watchdog o un errore di sistema. Questa parte finale delle ultime righe di log è stata salvata appena prima del riavvio e non sarà più disponibile dopo la chiusura.'
  },

  // Privacy
  privacy: {
    title: 'Privacy',
    updateCheck: 'Gli aggiornamenti automatici e i controlli del firmware si connettono al server raw.githubusercontent.com / github.com. Il tuo indirizzo IP viene trasmesso per verificare la disponibilità di nuove versioni.',
  },

  // Changelog
  changelog: {
    title: 'Registro delle Modifiche',
    loading: 'Caricamento...',
    fetching: 'Recupero registro modifiche da GitHub...',
    error: 'Impossibile caricare il registro delle modifiche',
    fetchError: 'Impossibile recuperare il registro delle modifiche. Controlla la tua connessione internet.',
    retry: 'Riprova',
    close: 'Chiudi',
    viewOnGithub: 'Vedi su GitHub'
  },

  // Supporter
  supporter: {
    title: 'Chiave sostenitore',
    active: 'Sostenitore',
    badgeLabel: 'Licenza di sostegno',
    intro: 'Sostieni questo progetto con un piccolo contributo (es. un caffè) e ricevi una chiave personale. Il badge è puramente estetico — nessuna funzione viene bloccata.',
    thanksTitle: 'Grazie per il tuo supporto!',
    thanksBody: 'Il tuo stato di sostenitore è valido fino al {date}.',
    remove: 'Rimuovi chiave',
    placeholder: 'XXXX-XXXX-XXXX-XXXX',
    activate: 'Attiva',
    activated: 'Chiave sostenitore attivata — grazie per il supporto!',
    activateFailed: 'Impossibile attivare la chiave sostenitore.',
    removed: 'Chiave sostenitore rimossa.',
    revokedTitle: 'Questa chiave è stata revocata',
    revokedBody: 'La chiave è stata aggiunta alla mia lista di revoca e non è più valida. Se è un errore, contattami.',
    invalid: 'Chiave non valida — controlla l’inserimento.',
    expired: 'Questa chiave è scaduta il {date}.',
    validHint: 'Valida — scade il {date}.',
    footnote: 'Ogni chiave è unica e valida per un periodo limitato. Viene salvata su questo dispositivo.',
    chipActive: 'Sostenitore attivo',
    chipInactive: 'Sostieni il progetto',
    chipTooltip: 'Sostenitore — valido fino al {date}',
    chipInactiveTooltip: 'Questo progetto dipende dal sostegno volontario — diventa sostenitore',
    expiredPromptTitle: 'La tua chiave sostenitore è scaduta',
    expiredPromptSupport: 'Sostieni di nuovo',
    expiredPromptLater: 'Forse più tardi',
    expiredPromptBody: 'La tua chiave sostenitore è scaduta. Con un piccolo contributo (es. un caffè) sostieni lo sviluppo continuo di questo progetto e ricevi una nuova chiave come ringraziamento. Grazie! ❤'
  }
}
