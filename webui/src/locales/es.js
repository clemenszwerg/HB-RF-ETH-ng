export default {
  // Common
  common: {
    enabled: 'Habilitado',
    disabled: 'Deshabilitado',
    save: 'Guardar',
    cancel: 'Cancelar',
    close: 'Cerrar',
    loading: 'Cargando...',
    changing: 'Cambiando...',
    error: 'Error',
    success: 'Éxito',
    yes: 'Sí',
    no: 'No',
    ok: 'OK',
    unknownError: 'Error desconocido',
    copyFailed: 'Error al copiar',
    clear: 'Borrar',
    network: {
      serverErrorTitle: 'Error del servidor',
      serverError: 'Error del servidor: {status}',
      timeoutTitle: 'Tiempo de espera agotado',
      timeout: 'La solicitud tardó demasiado. Inténtelo de nuevo.',
      connectionErrorTitle: 'Error de conexión',
      connectionError: 'No se puede conectar al dispositivo. Compruebe la red.'
    }
  },

  // App-level chrome (footer copyright, sponsor button)
  app: {
    footerCopyright: 'HB-RF-ETH-ng {version} © 2025-2026 Xerolux',
    sponsor: 'Patrocinar',
    followOnX: 'Seguir en X'
  },

  updateSuccess: {
    title: 'Actualización correcta',
    message: 'El firmware se actualizó correctamente a la versión {version}.'
  },

  // Header Navigation
  nav: {
    home: 'Inicio',
    settings: 'Configuración',
    firmware: 'Firmware',
    updates: 'Updates',
    documentation: 'Documentación',
    monitoring: 'Monitoreo',
    systemlog: 'Registro del sistema',
    about: 'Acerca de',
    login: 'Iniciar sesión',
    logout: 'Cerrar sesión',
    toggleTheme: 'Cambiar tema',
    language: 'Idioma',
    mainMenu: 'Menú principal',
    groupOverview: 'Vista general',
    groupSystem: 'Sistema',
    groupInfo: 'Info'
  },

  // Login Page
  login: {
    title: 'Por favor inicie sesión',
    subtitle: 'Por favor ingrese su nombre de usuario y contraseña para continuar',
    username: 'Nombre de usuario',
    usernamePlaceholder: 'Usuario',
    password: 'Contraseña',
    passwordPlaceholder: 'Contraseña',
    login: 'Acceder',
    loginFailed: 'Inicio de sesión fallido',
    invalidCredentials: 'Credenciales inválidas',
    loginError: 'El inicio de sesión no fue exitoso.',
    usernameRequired: 'El nombre de usuario es obligatorio',
    passwordRequired: 'Se requiere contraseña',
    loggingIn: 'Iniciando sesión...',
    forgotPassword: '¿Olvidó su contraseña?',
    passwordResetTitle: 'Restablecer contraseña',
    passwordResetPressButton: 'Pulse brevemente ahora el botón HM de la placa (GPIO34 / HM_BTN) para confirmar que tiene acceso físico.',
    passwordResetExpires: 'Ventana de tiempo: {seconds} segundos',
    passwordResetConfirmed: 'Botón de la placa detectado',
    passwordResetEnterNew: 'Por favor, establezca ahora una nueva contraseña de administrador.',
    newPasswordPlaceholder: 'Nueva contraseña',
    confirmPasswordPlaceholder: 'Repita la nueva contraseña',
    passwordResetSave: 'Guardar contraseña',
    passwordResetPolicy: 'Use entre 8 y 32 caracteres con mayúsculas, minúsculas y un número.',
    passwordResetMismatch: 'Las contraseñas no coinciden.',
    passwordResetExpired: 'La ventana de tiempo ha expirado. Por favor, inténtelo de nuevo.',
    passwordResetError: 'El restablecimiento de la contraseña no tuvo éxito.',
    footerCopyright: '© Xerolux 2026',
    githubLink: 'GitHub'
  },

  // Settings Page
  settings: {
    ledPatternOff: 'Apagado',
    ledPatternOn: 'Encendido',
    ledPatternBlink: 'Parpadeo',
    ledPatternBlinkInv: 'Parpadeo invertido',
    ledPatternFastBlink: 'Parpadeo rápido',
    ledPatternSlowBlink: 'Parpadeo lento',
    ledPatternBlink2x: 'Parpadeo 2x',
    ledPatternBlink3x: 'Parpadeo 3x',
    ledPatternBreathing: 'Respiración',
    ledPatternHeartbeat: 'Latido',
    ledPatternStrobe: 'Estroboscópico',
    ledProgramIdle: 'Inactivo',
    ledProgramCcuDisconnected: 'CCU desconectada',
    ledProgramCcuConnected: 'CCU conectada',
    ledProgramUpdateAvailable: 'Actualización disponible',
    ledProgramError: 'Error',
    ledProgramBooting: 'Arrancando',
    ledProgramUpdateInProgress: 'Actualización en curso',
    unsavedLeave: 'Tienes cambios sin guardar. ¿Salir de todos modos?',
    unsavedChanges: 'Cambios sin guardar',
    allSaved: 'Guardado',
    selected: 'Seleccionado: {name}',
    discardConfirm: '¿Descartar los cambios sin guardar y restaurar los últimos valores cargados?',
    discard: 'Descartar',
    restartError: 'Error al reiniciar el dispositivo',
    title: 'Configuración',
    ccuSettings: 'Conexión CCU',
    ccuIpAddress: 'Dirección IP de CCU',
    ccuIpHint: 'Por favor ingrese la dirección IP de su CCU para evitar el bloqueo de la conexión. El sistema se reiniciará después de guardar.',
    tabGeneral: 'General',
    tabNetwork: 'Red',
    tabTime: 'Hora',
    tabBackup: 'Copia de seguridad',
    tabDesign: 'Diseño',
    tabLicense: 'Licencia',
    tabExperimental: 'Experimental',
    advancedTitle: 'Avanzado',
    showExperimental: 'Mostrar funciones experimentales',
    showExperimentalHint: 'Revela entradas de menú, pestañas y ajustes experimentales. Los valores experimentales guardados se conservan al volver a ocultarlos.',
    showExperimentalWarning: 'Las funciones experimentales están inacabadas y pueden cambiar o no funcionar como se espera.',
    showExperimentalOn: 'Funciones experimentales visibles',
    showExperimentalOff: 'Funciones experimentales ocultas',
    changePassword: 'Cambiar contraseña',
    repeatPassword: 'Repetir contraseña',
    hostname: 'Nombre de host',

    // Security Settings
    security: 'Seguridad',
    adminUsername: 'Nombre de usuario administrador',
    adminUsernameHint: 'El valor predeterminado es admin. Puede cambiarse para gestores de contraseñas o varios dispositivos.',
    changePasswordHint: 'Cambie su contraseña de administrador para un acceso seguro',
    otaPassword: 'Contraseña OTA',
    otaPasswordHint: 'Se requiere contraseña separada para actualizaciones de firmware',
    changePasswordBtn: 'Cambiar contraseña',
    changeOtaPassword: 'Cambiar',
    setOtaPassword: 'Establecer contraseña',
    clearOtaPassword: 'Borrar',
    clearOtaPasswordConfirm: '¿Está seguro de que desea eliminar la contraseña OTA? Las actualizaciones de firmware no serán posibles hasta que se establezca una nueva contraseña.',
    clearOtaPasswordSuccess: 'La contraseña OTA ha sido eliminada.',
    clearOtaPasswordError: 'Error al eliminar la contraseña OTA',

    // Network Settings
    networkSettings: 'Configuración de red',
    dhcp: 'DHCP',
    ipAddress: 'Dirección IP',
    netmask: 'Máscara de red',
    gateway: 'Puerta de enlace',
    dns1: 'DNS primario',
    dns2: 'DNS secundario',

    // IPv6 Settings
    ipv6Settings: 'Configuración IPv6',
    enableIPv6: 'Habilitar IPv6',
    ipv6Mode: 'Modo IPv6',
    ipv6Auto: 'Automático (SLAAC)',
    ipv6Static: 'Estático',
    ipv6Address: 'Dirección IPv6',
    ipv6PrefixLength: 'Longitud del prefijo',
    ipv6Gateway: 'Puerta de enlace IPv6',
    ipv6Dns1: 'DNS IPv6 primario',
    ipv6Dns2: 'DNS IPv6 secundario',

    // Time Settings
    timeSettings: 'Configuración de hora',
    timesource: 'Fuente de tiempo',
    ntp: 'NTP',
    dcf: 'DCF',
    gps: 'GPS',
    ntpServer: 'Servidor NTP',
    dcfOffset: 'Desplazamiento DCF',
    gpsBaudrate: 'Baudrate GPS',

    // System Settings
    systemSettings: 'Configuración del sistema',
    ledBrightness: 'Brillo del LED',
    ledPrograms: 'Programas LED',
    ledProgramsHelp: 'Personaliza el comportamiento de los LED para los distintos estados del sistema.',
    allowPrerelease: 'Permitir actualizaciones tempranas (Beta/Alpha)',
    language: 'Idioma',

    // Messages
    saveSuccess: 'La configuración se guardó correctamente.',
    saveError: 'Se produjo un error al guardar la configuración.',
    restartTitle: 'Reinicio requerido',
    restartMessage: 'La configuración se ha guardado. ¿Desea reiniciar el dispositivo ahora para que los cambios surtan efecto?',
    restartNow: 'Reiniciar ahora',
    restartLater: 'Reiniciar más tarde',

    // Backup & Restore
    backupRestore: 'Copia de seguridad y restauración',
    backupInfo: 'Descargue una copia de seguridad de su configuración para restaurarla más tarde.',
    restoreInfo: 'Cargue un archivo de copia de seguridad para restaurar la configuración. El sistema se reiniciará después.',
    downloadBackup: 'Descargar copia de seguridad',
    restore: 'Restaurar',
    restoreBtn: 'Restaurar',
    download: 'Descargar',
    noFileChosen: 'Ningún archivo seleccionado',
    browse: 'Examinar',
    restoreConfirm: '¿Está seguro? La configuración actual se sobrescribirá y el sistema se reiniciará.',
    experimentalTitle: 'Experimental',
    experimentalEmptyTitle: 'Nada por aquí todavía',
    experimentalEmptyText: 'Actualmente no hay funciones experimentales disponibles. El Nuevo Diseño y la Sincronización de Reinicio son estándar para todos los dispositivos.',
    experimentalWarningTitle: 'Trabajo en curso',
    experimentalWarningText: 'Estas funciones están pensadas para pruebas. No hay garantía de funcionamiento ni de visualización.',
    experimentalDesign: 'Diseño de prueba',
    experimentalDesignHint: 'Activa el nuevo diseño experimental. La elección se guarda en el dispositivo y se conserva tras reiniciar.',
    flashPause: 'Sincronización de reinicio',
    flashPauseHint: 'Tras un reinicio provocado por una actualización de firmware o un cambio de ajustes, desconecta el enlace Ethernet durante 35 s antes de reiniciar, para que el watchdog de la CCU detecte la desconexión y reinicie limpiamente.',
    restoreSuccess: 'Configuración restaurada correctamente. Reiniciando el sistema...',
    restoreError: 'Error al restaurar la configuración',
    backupError: 'Error al descargar la copia de seguridad',
    validation: {
      invalidIpv4OrIpv6: 'Dirección IPv4 o IPv6 no válida',
      invalidIpv6: 'Dirección IPv6 no válida',
      minPrefix: 'Mín 1',
      maxPrefix: 'Máx 128',
      fixErrors: 'Corrige los campos resaltados.'
    }
  },

  // System Info
  sysinfo: {
    updatesRecovery: 'Actualizaciones y recuperación',
    liveStreamExport: 'Transmisión en vivo y exportación',
    networkTimeSetup: 'Configuración de red y hora',
    copied: '{label} copiado',
    title: 'Información del sistema',
    goodMorning: 'Buenos días',
    goodAfternoon: 'Buenas tardes',
    goodEvening: 'Buenas noches',
    serial: 'Número de serie',
    hostname: 'Nombre de host',
    boardRevision: 'Revisión de la placa',
    uptime: 'Tiempo de actividad',
     // Value is the cause of the last reboot, not a timestamp (Korrekturauftrag §1.5).
     resetReason: 'Motivo de reinicio',
    cpuUsage: 'Uso de CPU',
    memoryUsage: 'Uso de memoria',
    ethernetStatus: 'Conexión Ethernet',
    rawUartRemoteAddress: 'Conectado con CCU',
    localIP: 'Dirección IPv4',
    netmask: 'Máscara de red',
    gateway: 'Puerta de enlace',
    dns: 'Servidores DNS',
    ipv6Addresses: 'Direcciones IPv6',
    radioModuleType: 'Tipo de módulo de radio',
    radioModuleSerial: 'Número de serie',
    radioModuleFirmware: 'Versión de firmware',
    radioModuleBidCosRadioMAC: 'Dirección de radio (BidCoS)',
    radioModuleHmIPRadioMAC: 'Dirección de radio (HmIP)',
    radioModuleSGTIN: 'SGTIN',
    version: 'Versión',
    latestVersion: 'Última versión',
    memory: 'Uso de memoria',
    cpu: 'Uso de CPU',
    temperature: 'Temperatura',
    voltage: 'Voltaje de alimentación',
    ethernet: 'Ethernet',
    connected: 'Conectado',
    disconnected: 'Desconectado',
    speed: 'Velocidad',
    duplex: 'Dúplex',
    radioModule: 'Módulo de radio',
    moduleType: 'Tipo de módulo',
    firmwareVersion: 'Versión de firmware',
    bidcosMAC: 'MAC de radio BidCoS',
    hmipMAC: 'MAC de radio HmIP',
    system: 'Sistema',
    network: 'Red',
    down: 'Abajo',
    mbits: 'Mbit/s',
    online: 'En línea',
    offline: 'Fuera de línea',
    dashboardTitle: 'Estado del sistema',
    monitoringSummary: 'MQTT, CheckMK, Prometheus, Syslog y notificaciones',
    updateAvailableShort: 'Actualizar {version}',
    lan: 'LAN',
    boardSenseHint: 'Lectura bruta del ADC del pin de detección de revisión de placa. Comparar con las cuatro tensiones nominales del divisor identifica la revisión: Rev 1.10/1.11 PUB ≈ 550 mV, Rev 1.8 SK ≈ 1650 mV, Rev 1.8 PUB ≈ 2750 mV, Rev 1.10/1.11 SK ≈ 3050 mV.'
  },

  // System Overview page (dashboard / Diagnostics)
  systemOverview: {
    eyebrow: 'Diagnóstico',
    title: 'Resumen del sistema',
    subtitle: 'RAM, flash, particiones, registros e interfaz web activa de un vistazo.',
    freeRam: 'RAM libre',
    usedRam: 'RAM usada',
    totalRam: 'RAM total',
    minimumHeap: 'RAM mínima',
    largestBlock: 'Bloque más grande',
    flash: 'Flash',
    currentAvailable: 'Disponible actualmente',
    allocatableRam: 'RAM interna disponible para el heap',
    sinceBoot: 'Valor más bajo desde el arranque',
    contiguous: 'Asignación contigua más grande',
    physicalFlash: 'Almacenamiento flash detectado',
    runtime: 'Entorno de ejecución',
    runtimeHint: 'ESP-IDF, hardware y último reinicio',
    idf: 'ESP-IDF',
    target: 'Plataforma de destino',
    cores: 'Núcleos de CPU',
    chipRevision: 'Revisión del chip',
    resetReason: 'Último motivo de reinicio',
    psram: 'PSRAM',
    psramTotal: 'PSRAM total',
    psramFree: 'PSRAM libre',
    partitions: 'Particiones OTA',
    partitionsHint: 'Partición de firmware activa y siguiente',
    runningPartition: 'Activa',
    runningSize: 'Tamaño activo',
    nextPartition: 'Próxima actualización',
    nextSize: 'Tamaño de actualización',
    webuiHint: 'Origen y almacenamiento del nuevo diseño',
    source: 'Origen',
    version: 'Versión',
    partitionSize: 'Tamaño de partición',
    used: 'Usado',
    logs: 'Búfer de registro',
    logsHint: 'Memoria, crash tail y streams en vivo activos',
    capture: 'Captura',
    active: 'Activo',
    inactive: 'Inactivo',
    bufferSize: 'Tamaño del búfer',
    availableLogs: 'Registros almacenados',
    totalStream: 'Total escrito',
    subscribers: 'Suscriptores',
    crashTail: 'Crash tail',
    available: 'Disponible',
    notAvailable: 'No disponible',
    onDemandHint: 'Todos los valores se leen solo al abrir o actualizar manualmente. La página de emergencia independiente está disponible en',
    refresh: 'Actualizar'
  },

  // Update
  update: {
    available: 'Actualización disponible',
    updateNow: 'Actualizar ahora'
  },

  // Updates page (merged Firmware + WebUI entry, Korrekturauftrag §6)
  updates: {
    eyebrow: 'Actualizaciones',
    title: 'Actualizaciones',
    subtitle: 'El firmware y la interfaz web tienen versiones independientes y se actualizan por separado.',
    firmware: 'Firmware',
    webui: 'WebUI',
    checkNow: 'Buscar actualizaciones ahora',
    checkingNow: 'Buscando …',
    checkResultUpdatedTitle: 'Actualización disponible',
    checkResultUpdated: 'La versión {version} está disponible.',
    checkResultNoUpdateTitle: 'Todo está actualizado',
    checkResultNoUpdate: 'No hay ninguna versión más reciente disponible.',
    checkResultErrorTitle: 'Comprobación fallida',
    checkResultError: 'No se pudo completar la comprobación de actualizaciones.',
    checkResultCooldownTitle: 'Un momento',
    checkResultCooldown: 'Ya hay una comprobación en curso o el bloqueo de 60 segundos sigue activo.',
    checkResultSkippedTitle: 'Comprobación omitida',
    checkResultSkipped: 'El dispositivo omitió la comprobación por falta de memoria libre. Esto suele ocurrir mientras hay una sesión de CCU activa — inténtelo de nuevo más tarde.'
  },

  // Página de actualización de WebUI (pestaña separada de WebUI)
  webuiUpdate: {
    pageTitle: 'Actualizar la interfaz web',
    pageSubtitle: 'El firmware y la WebUI tienen versiones separadas. La instalación se realiza exclusivamente mediante una imagen WWW de 320 KB seleccionada manualmente.',
    webuiChip: 'WebUI',
    firmwareChip: 'Firmware',
    separateActive: 'WebUI independiente activa',
    embeddedFallback: 'Respaldo integrado activo',
    installedLabel: 'Versión de WebUI instalada',
    separateSource: 'De la partición WWW independiente',
    embeddedSource: 'Integrada en el firmware',
    storageLabel: 'Almacenamiento WWW',
    checkLabel: 'Comprobación de actualización',
    checkHint: 'Consulta en línea como máximo una vez cada 24 horas',
    availableKicker: 'WebUI disponible',
    downloadHeading: 'Descargar archivo WebUI',
    downloadHelp: 'El ESP solo descarga el pequeño manifiesto de actualización compartido. El archivo BIN se descarga directamente desde su navegador.',
    installedGridLabel: 'Instalada',
    availableGridLabel: 'Disponible',
    imageSizeLabel: 'Tamaño de la imagen',
    minFirmwareLabel: 'Firmware mínimo',
    upToDate: 'La WebUI instalada está actualizada.',
    downloadButton: 'Descargar BIN de WebUI',
    releaseLink: 'Versión en GitHub',
    reloadStatusButton: 'Recargar estado guardado',
    manualKicker: 'Instalación manual',
    uploadHeading: 'Subir imagen WWW',
    uploadHelp: 'Utilice aquí únicamente webui_*.bin o spiffs.bin con exactamente {size}.',
    chooseFile: 'Elegir BIN de WebUI',
    uploadInProgress: 'Transferiendo WebUI …',
    uploadExplainer: 'El dispositivo comprueba el tamaño de la partición, el manifiesto interno y, opcionalmente, SHA-256. Si la imagen no es válida o está incompleta, permanece activo el New Design integrado como respaldo.',
    installingButton: 'Instalando WebUI …',
    installButton: 'Instalar WebUI',
    clearSelectionButton: 'Borrar selección',
    unknownVersion: 'desconocida',
    neverChecked: 'Aún no comprobado',
    statusLoadError: 'No se pudo cargar el estado del dispositivo.',
    manifestNoWebui: 'El manifiesto de actualización guardado aún no contiene un archivo WebUI independiente.',
    manifestWrongDesign: 'La imagen WebUI proporcionada no pertenece al New Design.',
    manifestWrongSize: 'La WebUI ofrecida tiene un tamaño incorrecto. Se esperaba {expected}.',
    manifestWrongApi: 'Esta WebUI requiere una API de firmware distinta.',
    manifestFirmwareTooOld: 'El firmware es demasiado antiguo. Se requiere al menos {min}.',
    manifestLoadError: 'No se pudo cargar el estado de actualización guardado.',
    updateAvailableTitle: 'Actualización disponible',
    updateAvailableMessage: 'La versión {version} de WebUI está disponible.',
    skippedTitle: 'Comprobación omitida',
    skippedMessage: 'El dispositivo omitió la comprobación por falta de memoria libre. Suele ocurrir mientras hay una sesión de CCU activa — inténtelo de nuevo más tarde.',
    upToDateTitle: 'Todo está actualizado',
    upToDateMessage: 'No hay ninguna WebUI más reciente disponible.',
    cooldownTitle: 'Un momento',
    cooldownMessage: 'Ya hay una comprobación en curso o el bloqueo de 60 segundos sigue activo.',
    checkFailedTitle: 'Comprobación fallida',
    checkFailedMessage: 'No se pudo completar la comprobación de actualizaciones.',
    fileInvalidExtension: 'Archivo no válido: se requiere una imagen WebUI con la extensión .bin.',
    fileIsFirmware: 'Archivo incorrecto: es un archivo de firmware. Instálelo en Sistema → Firmware.',
    fileStatusMissing: 'Estado del dispositivo no disponible. Ejecute primero “Recargar estado guardado” e inténtelo de nuevo.',
    fileWrongSize: 'Tamaño de imagen incorrecto: se esperan exactamente {expected} ({bytes} bytes).',
    installSuccessTitle: 'WebUI instalada',
    installSuccessMessage: 'Versión activa de WebUI: {version}',
    uploadFailedTitle: 'Error al subir WebUI',
    uploadFailedMessage: 'Error al subir WebUI.'
  },

  // Firmware Update
  firmware: {
    jumpToOta: 'Ir a OTA',
    lastCheckAt: 'Última comprobación: {time}',
    noRecentCheck: 'Sin comprobaciones recientes',
    title: 'Firmware',
    subtitle: 'Actualizar el firmware del dispositivo',
    restarting: 'Reiniciando...',
    restartingText: 'El dispositivo se está reiniciando. La página se recargará automáticamente.',
    fileUpload: 'Carga de archivo',
    fileUploadHint: 'Cargar un archivo de firmware .bin',
    networkUpdate: 'Actualización de red',
    networkUpdateHint: 'Descargar desde URL',
    urlPlaceholder: 'https://ejemplo.com/firmware.bin',
    downloading: 'Descargando...',
    downloadInstall: 'Descargar e instalar',
    factoryReset: 'Restablecer',
    factoryResetHint: 'Restablecer a valores de fábrica',
    factoryResetTitle: 'Restablecimiento de fábrica',
    factoryResetMessage: '¿Realmente desea restablecer toda la configuración a los valores de fábrica?',
    factoryResetWarning: 'Se perderán todos los ajustes guardados. El dispositivo se reiniciará automáticamente después.',
    factoryResetConfirm: 'Restablecer a valores de fábrica',
    factoryResetError: 'El restablecimiento de fábrica falló.',
    otaProgress: 'Descargando actualización de firmware...',
    otaSuccess: 'Descarga exitosa. Instalando...',
    currentVersion: 'Versión actual',
    installedVersion: 'Versión instalada',
    versionInfo: 'Fork modernizado v2.2.5-Beta.8 por Xerolux (2025) - Basado en el trabajo original de Alexander Reinert.',
    updateAvailable: '¡Actualización disponible!',
    updateAvailableMessage: 'Hay una actualización disponible para la versión {latestVersion}.',
    upToDate: 'El firmware está actualizado',
    checkFailed: 'Error al comprobar actualización',
    lastCheck: 'Última comprobación',
    newVersionAvailable: '¡Nueva versión {version} disponible!',
    viewUpdate: 'Ver',
    onlineUpdate: 'Actualizar en línea',
    onlineUpdateConfirm: '¿Realmente desea descargar e instalar la actualización? El sistema se reiniciará automáticamente.',
    onlineUpdateStarted: 'Actualización iniciada. El dispositivo se reiniciará automáticamente una vez finalizado.',
    showReleaseNotes: 'Mostrar notas de la versión',
    releaseNotesTitle: 'Notas de la versión para v{version}',
    releaseNotesError: 'No se pudieron cargar las notas de la versión desde GitHub.',
    updateFile: 'Archivo de firmware',
    noFileChosen: 'Ningún archivo seleccionado',
    browse: 'Examinar',
    selectFile: 'Seleccionar o soltar archivo .bin',
    upload: 'Instalar firmware',
    restart: 'Reiniciar',
    restartHint: 'Reiniciar dispositivo',
    uploading: 'Cargando...',
    uploadSuccess: 'Actualización de firmware cargada correctamente. El sistema se reiniciará automáticamente en 3 segundos...',
    uploadError: 'Ocurrió un error.',
    updateSuccess: 'Firmware actualizado correctamente',
    updateError: 'Error al actualizar el firmware',
    warning: 'Advertencia: ¡No desconecte la alimentación durante la actualización!',
    restartConfirm: '¿Realmente desea reiniciar el sistema?',
    beta: 'Beta',
    betaChannel: 'Actualizaciones beta',
    betaChannelHint: 'Incluir versiones preliminares. Las versiones beta pueden ser inestables.',
    betaChannelOn: 'Actualizaciones beta activadas – verificación repetida',
    betaChannelOff: 'Actualizaciones beta desactivadas – verificación repetida',
    releaseNotesPreview: 'Notas de la versión',
    viewOnGithub: 'Ver en GitHub',
    noDownloadUrl: 'Ningún binario de firmware adjunto a esta versión. Vuelva a intentarlo más tarde o súbralo manualmente.',
    archiveTitle: 'Archivo de firmware',
    archiveHint: 'Instale una versión anterior, cambie de beta a estable o pruebe una versión concreta.',
    archiveFilter: 'Filtro del archivo de firmware',
    archiveSelect: 'Seleccionar versión de firmware',
    archiveStable: 'Estable',
    archiveBeta: 'Beta',
    archiveAll: 'Todas',
    archiveLoading: 'Cargando versiones...',
    archiveWarning: 'Cree una copia de seguridad antes de cambiar el firmware. Las versiones antiguas podrían no entender completamente los ajustes nuevos.',
    archiveLoadError: 'No se pudo cargar el archivo de firmware',
    archiveEmpty: 'No se encontraron versiones de firmware coincidentes.',
    archiveCurrent: 'Instalada',
    archiveInstalled: 'Instalada',
    archiveInstall: 'Instalar',
    archiveInstallConfirm: '¿Instalar la versión de firmware {version}? El dispositivo la descargará en línea y se reiniciará automáticamente. Cree antes una copia de seguridad.',
    archiveReleaseNotes: 'Changelog / notas de la versión',
    otaFailed: 'Error en la actualización OTA',
    lostConnection: 'Se perdió la conexión con el dispositivo durante la actualización',
    restartFlashPauseHint: 'La sincronización de reinicio está activa: el enlace Ethernet se mantendrá desconectado unos 35 s antes de que el dispositivo se reinicie realmente. La recarga de la página tardará un poco más de lo habitual.'
  },

  // Monitoring
  monitoring: {
    selfTestHint: 'Ejecuta una autocomprobación rápida para verificar la configuración actual.',
    selfTestTitle: 'Autocomprobación',
    testButton: 'Test',
    checkFinished: 'Comprobación finalizada',
    diagnosticFailed: 'La solicitud de diagnóstico falló',
    chipLabelCheckmk: 'CheckMK',
    chipLabelMqtt: 'MQTT',
    chipLabelPrometheus: 'Prometheus',
    chipLabelSyslog: 'Syslog',
    chipLabelNotify: 'Notificaciones',
    title: 'Monitoreo',
    description: 'Configure el monitoreo y las notificaciones para la puerta de enlace HB-RF-ETH.',
    resourceWarningTitle: 'Vigile los recursos',
    resourceWarningText: 'Hay {count} servicios de monitoreo activos. El ESP32 normalmente puede con ello, pero las conexiones TLS simultáneas, las comprobaciones/consulta de archivo de firmware y la OTA compiten por la memoria heap y la CPU; si surgen problemas, desactive servicios individuales o espacie las pruebas en el tiempo.',
    save: 'Guardar',
    saving: 'Guardando...',
    saveSuccess: '¡Configuración guardada correctamente!',
    saveError: '¡Error al guardar la configuración!',
    checkmk: {
      title: 'Agente CheckMK',
      enabled: 'Habilitar CheckMK',
      port: 'Puerto',
      portHelp: 'Predeterminado: 6556',
      allowedHosts: 'IPs de clientes permitidas',
      allowedHostsHelp: 'Direcciones IP separadas por comas (p. ej. "192.168.1.10,192.168.1.20") o "*" para todos'
    },
    mqtt: {
      title: 'Cliente MQTT',
      enabled: 'Habilitar MQTT',
      server: 'Servidor',
      serverHelp: 'Nombre de host o IP del broker MQTT',
      port: 'Puerto',
      portHelp: 'Predeterminado: 1883 (8883 para TLS/SSL)',
      user: 'Usuario',
      userHelp: 'Opcional: Nombre de usuario MQTT',
      password: 'Contraseña',
      passwordHelp: 'Opcional: Contraseña MQTT',
      topicPrefix: 'Prefijo del tema',
      topicPrefixHelp: 'Segmento raíz de cada topic MQTT que este dispositivo publica/suscribe. Predeterminado: hb-rf-eth-ng. Ejemplos: "prefijo/status/uptime", "prefijo/status/version", "prefijo/command/update". Visible en su broker MQTT (ej. MQTT Explorer), no en la interfaz Web.',
      haDiscoveryEnabled: 'Descubrimiento de Home Assistant',
      haDiscoveryPrefix: 'Prefijo de descubrimiento',
      haDiscoveryPrefixHelp: 'Predeterminado: homeassistant',
      commands: {
        title: 'Topics de comando',
        enableHelp: 'Permite el control remoto (reinicio, restablecimiento de fábrica, OTA) a través de MQTT. Sin token ni ACL del broker, cualquier cliente de la red puede controlar el dispositivo.',
        token: 'Token de comando',
        tokenPlaceholder: 'opcional – secreto compartido',
        tokenHelp: 'Si se define, la carga útil del comando debe coincidir exactamente con este token. Permitido: A-Z a-z 0-9 - _ .',
        tokenPresent: '✓ Presente – introduzca un nuevo valor para reemplazar',
        clear: 'Borrar',
        aclHint: 'Nota: sin ACL del broker + token, cualquier cliente MQTT puede reiniciar el dispositivo o iniciar una OTA. Configure en el broker una ACL que conceda permisos de publicación en el topic de comando solo a este dispositivo.',
      },
      serverRequired: 'Por favor, introduzca una dirección de servidor MQTT cuando MQTT esté habilitado.',
      tls: {
        title: 'TLS / SSL',
        enable: 'Habilitar TLS',
        enableHelp: 'Opcional: Proteger la conexión MQTT mediante TLS',
        skipVerify: 'Deshabilitar verificación de certificado',
        skipVerifyAlert: 'Advertencia: Sin verificación de certificado no se pueden detectar ataques man-in-the-middle. Use solo en entornos de prueba o con certificados autofirmados.',
        caCerts: 'Certificado CA (PEM)',
        caCertsHelp: 'Dejar vacío = usar el bundle de CA raíz Mozilla integrado del firmware',
        certfile: 'Certificado de cliente (mTLS, PEM)',
        certfileHelp: 'Opcional: certificado de cliente mTLS',
        keyfile: 'Clave de cliente (mTLS, PEM)',
        keyfileHelp: 'Opcional: clave de cliente mTLS',
        certPresent: '✓ Presente – ingrese un nuevo valor para reemplazar',
        clear: 'Borrar',
        loadFromFile: 'Cargar desde archivo',
        fileTooLarge: 'Archivo demasiado grande (máx. 16 KB).',
        fileReadError: 'No se pudo leer el archivo.',
        pemValid: '✓ Formato PEM reconocido ({count} bloque | {count} bloques).',
        pemTooLarge: 'Contenido demasiado grande (máx. 8 KB por campo).',
        keySingleBlock: 'El archivo de clave debe contener exactamente un bloque PRIVATE KEY.',
        encryptedKeyError: 'Las claves privadas cifradas no son compatibles. Exporte sin contraseña: openssl pkcs8 -topk8 -nocrypt -in key.pem -out key-plain.pem',
        invalidPemFormat: 'No se detectó contenido PEM válido (faltan marcadores BEGIN/END o no coinciden).',
        unexpectedPemLabel: 'Tipo de bloque PEM inesperado para este campo.',
        binaryFormatError: 'Formato binario (DER) detectado – convierta primero: openssl x509 -inform DER -in cert.der -out cert.pem',
        pkcs12Error: 'El contenedor PKCS#12 (.p12/.pfx) no está soportado. Extraiga: openssl pkcs12 -in bundle.p12 -nodes -out cert.pem',
        invalidBase64: 'El cuerpo PEM contiene caracteres no válidos.',
        mtlsRequiresBoth: 'Para mTLS se deben proporcionar tanto el certificado como la clave.'
      }
    },
    enable: 'Habilitar',
    allowedHosts: 'Hosts permitidos',
    prometheus: {
      title: 'Exportador Prometheus',
      port: 'Puerto',
      portHelp: 'Predeterminado: 9100',
      allowedHosts: 'IPs de clientes permitidas',
      allowedHostsHelp: 'Direcciones IP separadas por comas o "*" para todas'
    },
    syslog: {
      title: 'Reenvío de Syslog',
      server: 'Servidor',
      serverHelp: 'Nombre de host o IP del servidor Syslog',
      port: 'Puerto',
      portHelp: 'Predeterminado: 514',
      transport: 'Transporte',
      minSeverity: 'Severidad mín.',
      minSeverityHelp: 'Solo reenviar mensajes a partir de este nivel de severidad',
      hostname: 'Sobrescribir hostname',
      hostnameHelp: 'Vacío = usar el hostname del dispositivo'
    },
    notify: {
      title: 'Notificaciones de eventos',
      channels: 'Canales activos',
      channelWebhook: 'Webhook',
      channelTelegram: 'Telegram',
      channelEmail: 'Correo electrónico',
      cooldown: 'Enfriamiento (segundos)',
      cooldownHelp: 'Por tipo de evento solo se notifica una vez dentro de esta ventana',
      webhookSection: 'Webhook',
      webhookUrl: 'URL',
      webhookSecret: 'Secreto',
      telegramSection: 'Telegram',
      telegramToken: 'Token del bot',
      telegramChatId: 'ID de chat',
      smtpSection: 'Correo (SMTP)',
      smtpServer: 'Servidor',
      smtpPort: 'Puerto',
      smtpTls: 'Cifrado',
      smtpTlsNone: 'Ninguno',
      smtpTlsStarttls: 'STARTTLS',
      smtpTlsImplicit: 'TLS (implícito)',
      smtpUser: 'Usuario',
      smtpPassword: 'Contraseña',
      smtpFrom: 'Remitente',
      smtpTo: 'Destinatario',
      secretPresent: '✓ Guardado – introduzca uno nuevo para reemplazar'
    },
    diag: {
      unsupported: 'Destino de diagnóstico desconocido',
      checkmk: {
        disabled: 'CheckMK está desactivado',
        listening: 'El agente CheckMK escucha en el puerto TCP {port}',
        not_ready: 'CheckMK está activado, pero el listener no está listo'
      },
      mqtt: {
        disabled: 'MQTT está desactivado',
        tcp_ok: 'Conexión TCP a {host}:{port} correcta',
        tcp_failed: 'Error en la conexión TCP a {host}:{port}',
        tls_note: ' (TLS activado, validación de certificados no comprobada)'
      },
      prometheus: {
        disabled: 'Prometheus está desactivado',
        listening: 'El exportador Prometheus escucha en el puerto TCP {port}',
        not_ready: 'Prometheus está activado, pero el listener no está listo'
      },
      syslog: {
        disabled: 'El reenvío de Syslog está desactivado',
        tcp_ok: 'Conexión TCP a {host}:{port} correcta',
        tcp_failed: 'Error en la conexión TCP a {host}:{port}'
      },
      notify: {
        disabled: 'Las notificaciones están desactivadas',
        queued: 'Notificación de prueba para los canales 0x{mask} encolada'
      }
    }
  },

  // About Page
  about: {
    title: 'Acerca de',
    version: 'Versión 2.2.5-Beta.6',
    fork: 'Fork Modernizado',
    forkDescription: 'Esta versión es un fork modernizado por Xerolux (2025), basado en el firmware original HB-RF-ETH. Actualizado a ESP-IDF 6.0, cadenas de herramientas modernas y tecnologías WebUI actuales (Vue 3, Vite, Pinia).',
    original: 'Autor original',
    firmwareLicense: 'El',
    hardwareLicense: 'El',
    under: 'se publica bajo',
    description: 'Gateway LAN HomeMatic BidCoS/HmIP',
    author: 'Autor',
    license: 'Licencia',
    website: 'Sitio web',
    documentation: 'Documentación',
    support: 'Soporte',
    githubRepository: 'Repositorio de GitHub',
    copyrightXerolux: 'Copyright (c) 2025, Xerolux',
    copyrightOriginal: 'Copyright (c) 2022, Alexander Reinert'
  },

  // Third Party
  thirdParty: {
    title: 'Software de terceros',
    containsThirdPartySoftware: 'Este software contiene productos de software de terceros gratuitos utilizados bajo varias condiciones de licencia.',
    providedAsIs: 'El software se proporciona "tal cual" SIN NINGUNA GARANTÍA.'
  },

  // Change Password
  changePassword: {
    title: 'Cambio de contraseña requerido',
    eyebrow: 'Seguridad',
    subtitle: 'Asegure su cuenta',
    warningTitle: 'Requerido',
    requirementsTitle: 'Requisitos de contraseña:',
    reqMinLength: 'Al menos 8 caracteres',
    reqLettersNumbers: 'Debe contener mayúsculas, minúsculas y números',
    currentPassword: 'Contraseña actual',
    newPassword: 'Nueva contraseña',
    confirmPassword: 'Confirmar contraseña',
    changePassword: 'Cambiar contraseña',
    changeSuccess: 'Contraseña cambiada correctamente',
    changeError: 'Error al cambiar la contraseña',
    passwordMismatch: 'Las contraseñas no coinciden',
    passwordTooShort: 'La contraseña debe tener al menos 8 caracteres y contener mayúsculas, minúsculas y números.',
    passwordRequirements: 'Debe contener mayúsculas, minúsculas y números',
    passwordsDoNotMatch: 'Las contraseñas no coinciden',
    warningMessage: 'Este es su primer inicio de sesión o la contraseña todavía está establecida en "admin". Por razones de seguridad, debe cambiar la contraseña.',
    success: 'Contraseña cambiada correctamente',
    newPasswordPlaceholder: 'Ingrese nueva contraseña',
    currentPasswordPlaceholder: 'Ingrese contraseña actual',
    currentPasswordRequired: 'Se requiere contraseña actual',
    confirmPasswordPlaceholder: 'Confirmar nueva contraseña'
  },

  // OTA Password Modal
  otaPassword: {
    title: 'Establecer contraseña OTA',
    warningMessage: 'Establezca una contraseña separada para las actualizaciones de firmware. Esto es necesario para las actualizaciones OTA.',
    otaPassword: 'Contraseña OTA',
    otaPasswordPlaceholder: 'Ingrese contraseña OTA',
    confirmPassword: 'Confirmar contraseña',
    currentPasswordPlaceholder: 'Ingrese contraseña actual',
    currentPasswordRequired: 'Se requiere contraseña actual',
    confirmPasswordPlaceholder: 'Confirmar contraseña OTA',
    passwordTooShort: 'La contraseña debe tener al menos 8 caracteres',
    passwordRequirements: 'Debe contener mayúsculas, minúsculas y números',
    passwordsDoNotMatch: 'Las contraseñas no coinciden',
    requirementsTitle: 'Requisitos de contraseña:',
    reqMinLength: 'Al menos 8 caracteres',
    reqMixedCase: 'Mayúsculas y minúsculas',
    reqNumbers: 'Al menos un número',
    strengthWeak: 'Débil',
    strengthMedium: 'Medio',
    strengthGood: 'Bueno',
    strengthStrong: 'Fuerte'
  },

  // Sponsor
  sponsor: {
    title: 'Apoyar este proyecto',
    description: 'Si le gusta este proyecto y desea apoyar su desarrollo, puede utilizar una de las siguientes opciones.',
    thanks: '¡Gracias por su apoyo!',
    teslaReferral: 'Referencia Tesla'
  },

  // System Log
  systemlog: {
    subtitle: 'Filtra, pausa y exporta el registro en vivo del dispositivo.',
    paused: 'En pausa',
    live: 'En vivo',
    lines: 'líneas',
    newEntries: 'nuevas',
    pause: 'Pausar',
    resume: 'Reanudar',
    manualScroll: 'Desplazamiento manual',
    copy: 'Copiar',
    searchPlaceholder: 'Buscar en el registro',
    allLevels: 'Todos los niveles',
    levelError: 'Error',
    levelWarning: 'Advertencia',
    levelInfo: 'Info',
    levelDebug: 'Debug',
    noMatches: 'No hay líneas coincidentes',
    copiedVisible: 'Registro visible copiado',
    copiedLine: 'Línea copiada',
    copyVisibleFailed: 'No se pudo copiar el registro',
    copyLineFailed: 'No se pudo copiar la línea',
    title: 'Registro del sistema',
    description: 'Vista en vivo de la salida del registro del sistema con capacidad de descarga.',
    liveLog: 'Registro en vivo',
    autoScroll: 'Desplazamiento automático',
    enabled: 'Habilitado',
    disabled: 'Deshabilitado',
    enableFailed: 'No se pudo habilitar el búfer de registro (memoria insuficiente).',
    disabledMessage: 'El visor de registros está deshabilitado. Encienda el interruptor para ver los registros en vivo.',
    refresh: 'Actualizar',
    clear: 'Borrar',
    download: 'Descargar',
    empty: 'Aún no hay entradas de registro.',
    crashTitle: 'Registro de fallo recuperado',
    crashHint: 'El dispositivo se reinició tras un reset del watchdog o un error del sistema. Este fragmento de las últimas líneas de registro se guardó justo antes del reinicio y no volverá a estar disponible tras cerrarlo.'
  },

  // Privacy
  privacy: {
    title: 'Privacidad',
    updateCheck: 'Las actualizaciones automáticas y las comprobaciones de firmware se conectan al servidor raw.githubusercontent.com / github.com. Se transmite su dirección IP para comprobar la disponibilidad de nuevas versiones.',
  },

  // Changelog
  changelog: {
    title: 'Registro de cambios',
    loading: 'Cargando...',
    fetching: 'Obteniendo registro de cambios de GitHub...',
    error: 'Error al cargar el registro de cambios',
    fetchError: 'No se pudo obtener el registro de cambios. Por favor verifique su conexión a internet.',
    retry: 'Reintentar',
    close: 'Cerrar',
    viewOnGithub: 'Ver en GitHub'
  },

  // Supporter
  supporter: {
    title: 'Clave de apoyo',
    active: 'Patrocinador',
    badgeLabel: 'Licencia de apoyo',
    intro: 'Apoya este proyecto con una pequeña aportación (p. ej. un café) y recibe una clave personal. La insignia es puramente estética — nunca bloquea funciones.',
    thanksTitle: '¡Gracias por tu apoyo!',
    thanksBody: 'Tu estado de patrocinador es válido hasta el {date}.',
    remove: 'Quitar clave',
    placeholder: 'XXXX-XXXX-XXXX-XXXX',
    activate: 'Activar',
    activated: 'Clave de apoyo activada — ¡gracias por tu apoyo!',
    activateFailed: 'No se pudo activar la clave de apoyo.',
    removed: 'Clave de apoyo eliminada.',
    revokedTitle: 'Esta clave ha sido revocada',
    revokedBody: 'La clave se añadió a mi lista de revocación y ya no es válida. Si es un error, contáctame.',
    invalid: 'Clave no válida — revisa la entrada.',
    expired: 'Esta clave expiró el {date}.',
    validHint: 'Válida — expira el {date}.',
    footnote: 'Cada clave es única y válida por tiempo limitado. Se guarda en este dispositivo.',
    chipActive: 'Patrocinador activo',
    chipInactive: 'Apoyar proyecto',
    chipTooltip: 'Patrocinador — válido hasta {date}',
    chipInactiveTooltip: 'Este proyecto depende del apoyo voluntario — hazte patrocinador',
    expiredPromptTitle: 'Tu clave de apoyo ha expirado',
    expiredPromptSupport: 'Apoyar de nuevo',
    expiredPromptLater: 'Quizá más tarde',
    expiredPromptBody: 'Tu clave de apoyo ha expirado. Con una pequeña aportación (p. ej. un café) apoyas el desarrollo continuo de este proyecto y recibes una nueva clave como agradecimiento. ¡Gracias! ❤'
  }
}
